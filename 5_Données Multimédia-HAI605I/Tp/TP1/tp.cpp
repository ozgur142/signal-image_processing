...
— 1 : activation/désactivation de l’affichage du modèle chargé
— 2 : activation/désactivation de l’affichage du modèle généré,
...
//To complete
void setDonut(Mesh & o_mesh){
    o_mesh.vertices.clear();
    o_mesh.triangles.clear();
    o_mesh.normals.clear();

    // c = distance entre centre de donut et centre de tube
    // a = r de tube
    const float c = 0.9;
    const float a = 0.3;
    ...
    for(int i = 0; i < step+1; ++i){
        for(int j = 0; j < step; ++j){
            double u = i * 2*M_PI / step;
            double v = j * 2*M_PI / step;
    ...
    for (int i = 0; i < step; ++i) {
        for (int j = 0; j < step; ++j) {
            ...
            o_mesh.triangles.push_back(Triangle(a1, b1, c1));
            o_mesh.triangles.push_back(Triangle(a1, c1, d1));
        }
    }

    // Normales
    for (const Vec3& v : o_mesh.vertices)
        o_mesh.normals.push_back(v);
...
void setUnitSphere( Mesh & o_mesh, int nX=10, int nY=10)
{
    ...
    const double tetaP = 2*M_PI / nY;
    const double phiP = M_PI / nX;
    ...
    for (int i = 0; i < nX - 2; ++i) {
        for (int j = 0; j < nY; ++j) {
            ...
            o_mesh.triangles.push_back(Triangle(a, c, d));
        }
    }
...
void init () {
    ...
    glClearColor (0.2f, 0.2f, 0.3f, 1.0f);  //couleur d'arrier plan
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case '+':
        if (display_unit_sphere){
            g_nX += 5;
            g_nY += 5;
            setUnitSphere(unit_sphere, g_nX, g_nY);
        }
        break;
    case '-':
        if (g_nX >= 0 && g_nY >= 0 && display_unit_sphere){
            g_nX -= 5;
            g_nY -= 5;
            setUnitSphere(unit_sphere, g_nX, g_nY);
        }
